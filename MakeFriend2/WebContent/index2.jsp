...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$(".btn-primary").click(function(){
			...
			var contextId = $(this).attr('getContextId');
			...
			//console.log("发送给"+recId);
	      	$.ajax({
	      		dataType:'text',
	      		url:"pub1",
	      		data:{"context":context,"recId":recId},
	      		type:"post",
	      		async:true,
						...
						<div id="yinying">
							...
							<c:if test="${not empty CurrUser}">
								...
								<%-- <c:if test="${user.sex=='男' }">
									<a id="send" href="" class="btn" id="btn" data-toggle="modal" data-target="#myModal${user.id}"
									style="background-color: pink; margin-left: 90px">和他聊天</a>
								</c:if>
								<c:if test="${user.sex=='女' }">
									<a id="send" href="" class="btn" id="btn" data-toggle="modal" data-target="#myModal${user.id}"
									style="background-color: pink; margin-left: 90px">和她聊天</a>
								</c:if> --%>
								
								<a id="send" href="" class="btn" id="btn" data-toggle="modal" data-target="#myModal${user.id}"
									style="background-color: pink; margin-left: 90px">${user.sex=='男' ?'和他聊天':'和她聊天'}</a>
							    ...
							    <div class="modal-dialog">
							        <div class="modal-content">
							            ...
							            <div class="modal-footer">
								            	...
								            	<textarea id="con${user.id}" rows="3" cols="50" name="context" placeholder="请输入留言的内容"></textarea>
								                <button class="btn btn-primary" getContextId="con${user.id}"  id="${user.id}">发送消息${user.id}</button>
	...
	<div class="row" style="margin-left: 70px; margin-right: 70px;" id="cp" style="position:relative">
		<c:if test="${not empty success }">
			<c:forEach var="success" items="${success.data}">
				<div class="col-md-3">
					<a href="success"><img class="img-responsive" alt="Responsive image" style="border-radius:5px"
